...
namespace core::Graphics::Direct3D11 {
	...
	bool Texture2D::initialize(Device* const device, void const* const data, size_t const size, bool const mipmap) {
		assert(device);
		assert(data);
		assert(size > 0);
		m_device = device;
		m_mipmap = mipmap;
		m_source_data.assign(
	...
	bool Texture2D::createResource() {
		...
		auto const create_texture_from_encoded_memory = [&](uint8_t const* const src_data, size_t const src_size) {
		...
		if (m_data) {
			...
			if (!create_texture_from_encoded_memory(static_cast<uint8_t const*>(src->data()), src->size())) {
				return false;
			}
		}
		else if (!m_source_data.empty()) {
			if (!create_texture_from_encoded_memory(m_source_data.data(), m_source_data.size())) {
				return false;
			}
		}
		else {
...
namespace core::Graphics::Direct3D11 {
	...
	bool Device::createTextureFromMemory(void const* const data, size_t const size, bool const mipmap, ITexture2D** const pp_texture) {
		*pp_texture = nullptr;
		if (!data || size == 0) {
			return false;
		}
